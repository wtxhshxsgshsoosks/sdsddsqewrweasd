...
import { skillDetails, serviceDetails, hostingProjectDetails, minecraftProjectDetails } from './data/hoverDetails';
                  ...
                  <ul className="space-y-3">
                    {services.map((service, index) => (
                      <li key={index} className="service-item group">
                        <HoverDetail
                          title={service.text}
                          content={serviceDetails[service.text as keyof typeof serviceDetails]}
                        >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.languages.map(lang => (
                          <HoverDetail
                            key={lang}
                            title={lang}
                            content={skillDetails.languages[lang as keyof typeof skillDetails.languages]}
                          >
                            <span className="skill-tag">{lang}</span>
                          </HoverDetail>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.databases.map(db => (
                          <HoverDetail
                            key={db}
                            title={db}
                            content={skillDetails.databases[db as keyof typeof skillDetails.databases]}
                          >
                            <span className="skill-tag">{db}</span>
                          </HoverDetail>
                        ))}
                      </div>
                    </div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.techStack.map(tech => (
                          <HoverDetail
                            key={tech}
                            title={tech}
                            content={skillDetails.techStack[tech as keyof typeof skillDetails.techStack]}
                          >
                      ...
                      <div className="flex flex-wrap gap-2">
                        {skills.specialties.map(specialty => (
                          <HoverDetail
                            key={specialty}
                            title={specialty}
                            content={skillDetails.specialties[specialty as keyof typeof skillDetails.specialties]}
                          >
                            <span className="skill-tag">{specialty}</span>
                          </HoverDetail>
                        ))}
                      </div>
                    </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {hostingProjects.map((project, index) => (
                    <HoverDetail
                      key={project.name}
                      title={project.name}
                      content={hostingProjectDetails[project.name as keyof typeof hostingProjectDetails]}
                    >
                      <div className="network-item bg-tech-gray/30 rounded-lg p-6 border border-gray-800/30">
                        <div className="flex items-start space-x-3">
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  {networks.map((network, index) => (
                    <HoverDetail
                      key={network.name}
                      title={network.name}
                      content={minecraftProjectDetails[network.name as keyof typeof minecraftProjectDetails]}
                    >
                      <div className="network-item bg-tech-gray/30 rounded-lg p-6 border border-gray-800/30">
                        <div className="flex items-start space-x-3">
                          <span className="text-accent/50 font-mono text-sm">
